...
public class A04Main {
    ...
    private static void characterTable(final BST<Character> tree) {
    	...
    	ArrayList<CountedItem<Character>> data = tree.inOrder();
    	
    	
    	for (CountedItem<Character> c : data) {
    		
    		total += c.getCount();
    	}
    	
    	x.append(String.format("Char    Count Percent\n"));
    	for (CountedItem<Character> c : data) {
    		x.append(String.format("%4s %,8d %7.2f\n", c.getItem(),c.getCount(),(c.getCount()/total)*100));
    	}
    ...
    private static void fillTree(final BST<Character> tree, final String string) {
    	...
    	final String string1 = string.toUpperCase();
    	for (Character letter : string1.toCharArray()) {
    		if (ALPHABET.contains(letter.toString())) {
    			tree.insert(new CountedItem<Character>(letter));
    		}
    	}
    }
    ...
    private static int retrieve(final BST<Character> tree, final Scanner fileScan) {
		...
		while (fileScan.hasNextLine()) {
			...
			for (Character letter : nextline.toCharArray()) {

				if (ALPHABET.contains(letter.toString())) {
					
					tree.retrieve(new CountedItem<Character>(letter));
					comp += tree.getComparisons(); 

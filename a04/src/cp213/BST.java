...
public class BST<T extends Comparable<T>> {
    ...
    protected boolean equalsAux(final TreeNode<T> source, final TreeNode<T> target) {
		...
		else if (source.getItem().compareTo(target.getItem()) == 0 && source.getHeight() == target.getHeight()
    ...
    protected TreeNode<T> insertAux(TreeNode<T> node, final CountedItem<T> data) {
		...
		if (node == null) {
			...
			int comp = node.getItem().compareTo(data);
    ...
    protected boolean isValidAux(final TreeNode<T> node, TreeNode<T> minNode, TreeNode<T> maxNode) {
		...
		if (node == null) {
			valid = true;
		} else if (node.getItem().compareTo(minNode.getItem()) >= 0 && node.getItem().compareTo(maxNode.getItem()) <= 0
				&& isValidAux(node.getLeft(), minNode, node) && isValidAux(node.getRight(), maxNode, node)) {
			valid = true;
		}
		return valid;
	}
...
contains = this.retrieve(key) != null;
	return contains;
    }
